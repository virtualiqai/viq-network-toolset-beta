...
This is a plain-English summary of how you may use Virtual IQ AI NetOps Toolset. The formal legal agreement is in [LICENSE](./LICENSE). If anything here conflicts with the LICENSE, the LICENSE controls.
...
You can install and use Virtual IQ AI NetOps Toolset for free if you are using it for:
...
You need a separate paid commercial license from us if you are using the toolset for:
...
### 🚫 Don't Scan Networks You Don't Own

This is the most important rule. **Only point this tool at networks, devices, and systems you own or have explicit written permission to access.**
...
- ❌ A neighbor's WiFi — not fine
- ❌ A coffee shop's network — not fine
- ❌ Any network you don't have explicit authorization to access — not fine

Unauthorized network scanning is illegal in most countries. In the United States it violates the **Computer Fraud and Abuse Act (18 U.S.C. § 1030)**, which carries serious criminal and civil penalties. If you misuse this tool, the consequences are entirely yours. We have no way to know what network you're pointing it at, and we will not be on the hook for your misuse.
...
The binary is intentionally obfuscated and encrypted. Don't try to extract the source code, deobfuscate the protections, or build a clone. If you want to know how something works, ask — we're often happy to share at a high level.

### 🚫 Don't Redistribute

Don't repackage, mirror, host on torrent sites, or share the binary outside the official GitHub release page. Always direct people to the official download link so they get the genuine, current version.
...
1. **Authorization.** You confirm you have permission to interact with every device, host, or network you target.
...
3. **Production impact.** Network probing carries risk. Test in lab environments first. If you take down a production switch with a Switch Health probe, that's on you — not us.
...
5. **Backups.** Always have current backups of any device configuration before running interactive tools like SSH Terminal.
...
This software is free. It comes "as is." We do not promise it will work in your environment, will not have bugs, will not crash, or will produce accurate results in every situation. We are not responsible for any damages, outages, data loss, fines, or other harm arising from your use of the software.

This is standard practice for free software. Major vendors of free network tools (Wireshark, Nmap, the Python ecosystem, and even paid products' free editions) operate under similar terms. If a free toolset came with full enterprise warranty, it would not be free.
...
2. **You take on legal responsibility.** If a third party sues us because of something you did with the tool, you agree to defend us and cover the costs (this is called *indemnification*; see LICENSE Section 9).
...
Email **info@virtualiqai.com**. We're a small company and we read everything.
